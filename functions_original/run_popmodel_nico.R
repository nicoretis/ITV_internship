...
#Only for shape sigmoid we have a differentiation for survival or fecund, I have to take a look over the test_sigmoid.R
  if (shape == "sigmoid") {
...

{
  ...
  for (t in 2:sum_years) {
    ...
    #Very simple: we calculate first the trait and then the survival for both juvenile and adult; only for the adult we calculate as well the fecundity, depending wheter their demo.type is on "survival" or "reproduction".
